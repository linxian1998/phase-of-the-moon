...
const { curr } = storeToRefs(usePlane());
// 动画函数
let cube;
let cubeBoundingBoxSize; // 存储边界框大小
let startTime
let plane
let index = 0
let canvas;
...
function animate() {
  ...
  const elapsedTimeMapped = elapsedTime / (duration / (2 * Math.PI)); // 将 elapsedTime 映射到 0 到 2π
  // 使用 Math.sin 函数计算 y 位置的变化
  const yAmplitude = 50; // y 轴上的振幅（最大变化量）
  const centerY = 0; // y 轴上的中心点
  const currentY = centerY + yAmplitude * Math.sin(elapsedTimeMapped - Math.PI / 2); // 通过调整 -Math.PI/2 使运动从上升开始
  const targetX = canvas?.offsetWidth - 50;

  const currentX = -canvas?.offsetWidth / 2 + 50 + targetX * (elapsedTime / duration); // 防止超过目标位置
...
onMounted(() => {
  ...
  canvas = document.getElementById('cannn');
  console.log(canvas);
...
// 用于显示捕捉画面的平面
  ...
  // plane.position.x = -canvas?.offsetWidth/2
  scene.add(plane);
  // 计算边界框
  // 设置相机位置
  ...
  // 创建一个环境光
  const ambientLight = new THREE.AmbientLight(0xffffff, 1); // 柔和的白色光
  scene.add(ambientLight);

 
  renderer.render(scene, camera);
  // 开始动画
  startTime = performance.now(); // 在动画开始时设置 startTime
