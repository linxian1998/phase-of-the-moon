...
void main(){
    ...
    radial=pow(radial,3.);
    float brightness=(1.+radial*.93)*radial*.9;
...
`;
const objMaterial = {
  sunShapeMaterial: undefined,
  sunNoiseMaterial: undefined
}
function createSunShapeMaterial() {
  ...
  objMaterial.sunShapeMaterial = sunShapeMaterial
  return objMaterial.sunShapeMaterial
}

// 创建太阳
function createSun() {
  const geometry = new THREE.SphereGeometry(400, 100, 100);
  const material = createSunShapeMaterial();
...
let cubeCamera;
let cubeScene;
function createSunNoiseMaterial() {
  ...
  objMaterial.sunNoiseMaterial = sunNoiseMaterial
  return objMaterial.sunNoiseMaterial
}
// 创建立方体离屏渲染目标，将其作为太阳本体的噪声贴图
function createCubeRt() {
  cubeRt = new THREE.WebGLCubeRenderTarget(256);
  cubeCamera = new THREE.CubeCamera(0.1, 10, cubeRt);
  cubeScene = new THREE.Scene();
  const geometry = new THREE.SphereGeometry(1, 100, 100);
  const material = createSunNoiseMaterial();
  let mesh = new THREE.Mesh(geometry, material);
  mesh.position.copy(new THREE.Vector3(0, 0, 0));
  cubeScene.add(mesh);
}
// // 创建太阳环材质
// function createSunRingMaterial() {
...
//     }
//   });
//   return sunRingMaterial;
// }
// // 创建太阳环
// function createSunRing() {
//   const geometry = new THREE.SphereGeometry(480, 100, 100);
//   const material = createSunRingMaterial();
//   let mesh = new THREE.Mesh(geometry, material);
//   return mesh;
// }
...
function update(render) {
  ...
  if (objMaterial.sunNoiseMaterial && objMaterial.sunShapeMaterial) {
    cubeCamera.update(render, cubeScene);
    objMaterial.sunNoiseMaterial.uniforms.uTime.value = elapsedTime;
    // this.sunNoiseMaterial.uniforms.uMouse.value = mousePos;
    objMaterial.sunShapeMaterial.uniforms.uTime.value = elapsedTime;
    // this.sunShapeMaterial.uniforms.uMouse.value = mousePos;
    objMaterial.sunShapeMaterial.uniforms.uNoiseTexture.value = cubeRt.texture;
  }
}

export { createSun, createCubeRt, update };
